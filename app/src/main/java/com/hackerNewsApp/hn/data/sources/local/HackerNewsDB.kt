...
import com.hackerNewsApp.hn.data.sources.local.converters.ListConverters
  ...
  companion object {
    ...
    fun getInstance(context: Context, isInMemory: Boolean = false): HackerNewsDB {
      return if (isInMemory) {
        Room.inMemoryDatabaseBuilder(context.applicationContext, HackerNewsDB::class.java).allowMainThreadQueries().build()
      } else Room.databaseBuilder(context.applicationContext, HackerNewsDB::class.java, db).build()
    }
  }
}
